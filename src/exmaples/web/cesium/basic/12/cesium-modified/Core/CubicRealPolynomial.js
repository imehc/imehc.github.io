 ...
 */
CubicRealPolynomial.computeDiscriminant = (a, b, c, d) => {
...
function computeRealRoots(a, b, c, d) {
	...
	if (discriminant < 0.0) {
		...
		const x = temp1 / 2.0;
		const p = x < 0.0 ? -((-x) ** (1.0 / 3.0)) : x ** (1.0 / 3.0);
 ...
 */
CubicRealPolynomial.computeRealRoots = (a, b, c, d) => {
	//>>includeStart('debug', pragmas.debug);
	if (typeof a !== "number") {
		throw new DeveloperError("a is a required number.");
	}
	...
	if (a === 0.0) {
		...
		if (c === 0.0) {
			...
			ratio = -d / a;
			const root =
				ratio < 0.0 ? -((-ratio) ** (1.0 / 3.0)) : ratio ** (1.0 / 3.0);
