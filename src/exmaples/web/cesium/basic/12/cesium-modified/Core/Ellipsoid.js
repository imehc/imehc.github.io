 ...
 */
Ellipsoid.clone = (ellipsoid, result) => {
 ...
 * @see Ellipsoid.UNIT_SPHERE
 */
Ellipsoid.fromCartesian3 = (cartesian, result) => {
	if (!defined(result)) {
 ...
 */
Ellipsoid.pack = (value, array, startingIndex) => {
	//>>includeStart('debug', pragmas.debug);
	Check.typeOf.object("value", value);
 ...
 */
Ellipsoid.unpack = (array, startingIndex, result) => {
	//>>includeStart('debug', pragmas.debug);
	Check.defined("array", array);
	//>>includeEnd('debug');
