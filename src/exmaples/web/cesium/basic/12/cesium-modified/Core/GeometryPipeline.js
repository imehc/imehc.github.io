...
GeometryPipeline.createAttributeLocations = (geometry) => {
	...
	for (const name in attributes) {
		if (Object.hasOwn(attributes, name) && !defined(indices[name])) {
			indices[name] = j++;
		}
	}
...
GeometryPipeline.reorderForPreVertexCache = (geometry) => {
	...
	if (defined(indices)) {
		...
		for (const property in attributes) {
			if (
				Object.hasOwn(attributes, property) &&
...
function copyVertex(destinationAttributes, sourceAttributes, index) {
	for (const attribute in sourceAttributes) {
		if (
			Object.hasOwn(sourceAttributes, attribute) &&
...
function findAttributesInAllGeometries(instances, propertyName) {
	...
	for (name in attributes0) {
		if (
			Object.hasOwn(attributes0, name) &&
...
const interpolateAndPackBoolean = (
	i0,
	i1,
	i2,
	coords,
