 ...
 * @returns {number} The sign of value.
 */
CesiumMath.signNotZero = (value) => (value < 0.0 ? -1.0 : 1.0);
 ...
 */
CesiumMath.toSNorm = (value, rangeMaximum) => {
 ...
 */
CesiumMath.fromSNorm = (value, rangeMaximum) => {
	rangeMaximum = rangeMaximum ?? 255;
	return (
 ...
 */
CesiumMath.normalize = (value, rangeMinimum, rangeMaximum) => {
 ...
 */
CesiumMath.lerp = (p, q, time) => (1.0 - time) * p + time * q;
 ...
 */
CesiumMath.convertLongitudeRange = (angle) => {
	//>>includeStart('debug', pragmas.debug);
	if (!defined(angle)) {
 ...
 */
CesiumMath.clampToLatitudeRange = (angle) => {
	//>>includeStart('debug', pragmas.debug);
	if (!defined(angle)) {
		throw new DeveloperError("angle is required.");
	}
 ...
 */
CesiumMath.negativePiToPi = (angle) => {
	//>>includeStart('debug', pragmas.debug);
	if (!defined(angle)) {
		throw new DeveloperError("angle is required.");
	}
 ...
 */
CesiumMath.equalsEpsilon = (left, right, relativeEpsilon, absoluteEpsilon) => {
	//>>includeStart('debug', pragmas.debug);
	if (!defined(left)) {
 ...
 */
CesiumMath.lessThan = (left, right, absoluteEpsilon) => {
	//>>includeStart('debug', pragmas.debug);
	if (!defined(left)) {
 ...
 */
CesiumMath.lessThanOrEquals = (left, right, absoluteEpsilon) => {
	//>>includeStart('debug', pragmas.debug);
	if (!defined(left)) {
		throw new DeveloperError("first is required.");
	}
 ...
 *          values are nearly equal.
 */
CesiumMath.greaterThan = (left, right, absoluteEpsilon) => {
	//>>includeStart('debug', pragmas.debug);
	if (!defined(left)) {
		throw new DeveloperError("first is required.");
	}
 ...
 *          the values are nearly equal.
 */
CesiumMath.greaterThanOrEquals = (left, right, absoluteEpsilon) => {
	//>>includeStart('debug', pragmas.debug);
	if (!defined(left)) {
		throw new DeveloperError("first is required.");
	}
 ...
 */
CesiumMath.incrementWrap = (n, maximumValue, minimumValue) => {
 ...
 */
CesiumMath.previousPowerOfTwo = (n) => {
	//>>includeStart('debug', pragmas.debug);
	if (typeof n !== "number" || n < 0 || n > 4294967295) {
		throw new DeveloperError("A number between 0 and (2^32)-1 is required.");
	}
 ...
 */
CesiumMath.clamp = (value, min, max) => {
	//>>includeStart('debug', pragmas.debug);
 ...
 */
CesiumMath.nextRandomNumber = () => randomNumberGenerator.random();
 ...
 */
CesiumMath.randomBetween = (min, max) =>
	CesiumMath.nextRandomNumber() * (max - min) + min;
 ...
 */
CesiumMath.chordLength = (angle, radius) => {
	//>>includeStart('debug', pragmas.debug);
	if (!defined(angle)) {
		throw new DeveloperError("angle is required.");
	}
 ...
 */
CesiumMath.logBase = (number, base) => {
	//>>includeStart('debug', pragmas.debug);
	if (!defined(number)) {
...
CesiumMath.cbrt =
	...
	function cbrt(number) {
		const result = Math.abs(number) ** (1.0 / 3.0);
 ...
 * @private
 */
CesiumMath.fog = (distanceToCamera, density) => {
 ...
 */
CesiumMath.fastApproximateAtan = (x) => {
	//>>includeStart('debug', pragmas.debug);
 ...
 */
CesiumMath.fastApproximateAtan2 = (x, y) => {
	//>>includeStart('debug', pragmas.debug);
	Check.typeOf.number("x", x);
