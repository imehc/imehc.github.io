 ...
 * @private
 */
Resource.createIfNeeded = (resource) => {
 ...
 * @returns {Promise<ArrayBuffer>|undefined} a promise that will resolve to the requested data when loaded. Returns undefined if <code>request.throttle</code> is true and the request does not have high enough priority.
 */
Resource.fetchArrayBuffer = (options) => {
...
Resource.prototype._fetchImage = function (options) {
	...
	request.requestFunction = () => {
		...
		if (!this.isDataUri && !this.isBlobUri) {
			crossOrigin = this.isCrossOriginUrl;
		}
	...
	return promise.catch((e) => {
		...
		return this.retryOnError(e).then((retry) => {
			if (retry) {
...
function fetchJsonp(resource, callbackParameterName, functionName) {
	...
	return promise.catch((e) => {
		...
		return resource.retryOnError(e).then((retry) => {
			if (retry) {
				// Reset request so it can try again
				request.state = RequestState.UNISSUED;
				request.deferred = undefined;
...
Resource.prototype._makeRequest = function (options) {
	...
	request.requestFunction = () => {
		...
		const headers = combine(options.headers, this.headers);
 ...
 */
Resource.createImageBitmapFromBlob = (blob, options) => {
...
Resource._Implementations.loadAndExecuteScript = (
	...
	loadAndExecuteScript(url, functionName).catch((e) => {
		deferred.reject(e);
	});
