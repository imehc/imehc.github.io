...
function RenderState(renderState) {
	...
	// 只在 ContextLimits 已初始化时才检查 lineWidth 范围
	if (
	...
	if (defined(this.viewport)) {
		...
		// 只在 ContextLimits 已初始化时才检查 viewport 大小
		if (
			ContextLimits.maximumViewportWidth > 0 &&
			this.viewport.width > ContextLimits.maximumViewportWidth
		) {
			throw new DeveloperError(
		...
		if (
			ContextLimits.maximumViewportHeight > 0 &&
			this.viewport.height > ContextLimits.maximumViewportHeight
		) {
			throw new DeveloperError(
 ...
 * @private
 */
RenderState.fromCache = (renderState) => {
 ...
 * @private
 */
RenderState.getCache = () => renderStateCache;
...
RenderState.apply = (gl, renderState, passState) => {
...
RenderState.getState = (renderState) => {
	//>>includeStart('debug', pragmas.debug);
