...
function generateDependencies(currentNode, dependencyNodes) {
	...
	if (defined(czmMatches) && czmMatches !== null) {
		// remove duplicates
		czmMatches = czmMatches.filter(
			(elem, pos) => czmMatches.indexOf(elem) === pos,
		);
...
ShaderSource.replaceMain = (source, renamedMain) => {
...
ShaderSource.createPickVertexShaderSource = (vertexShaderSource) => {
...
ShaderSource.findNormalVarying = (shaderSource) => {
...
ShaderSource.findPositionVarying = (shaderSource) =>
	findFirstString(shaderSource, positionVaryingNames);
