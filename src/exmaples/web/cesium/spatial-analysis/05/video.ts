...
export default class Video {
    ...
    public clearAll() {
        const dd = this.viewer.entities.values;
    ...
    public drawVideo() {
        ...
        // 获取视频DOM元素 (如果配置中提供了video ID)
        if (typeof this.config.video === "string") {
            this.video_dom = document.getElementById(this.config.video) as HTMLVideoElement;
        }
    ...
    public action() {
        this.handler.setInputAction((movement: { position: Cartesian2 }) => {
            const cartesian = this.getCurrentMousePosition(
                this.viewer.scene,
                movement.position,
            );
            if (!cartesian) {
    ...
    public createLightCamera() {
        ...
        if (hr > vr) {
            (this.lightCamera.frustum as PerspectiveFrustum).fov = hr;
        } else {
            (this.lightCamera.frustum as PerspectiveFrustum).fov = vr;
        }
    ...
    public drawFrustumOutline() {
        ...
        const videoGeometryInstance1 = new GeometryInstance({
            geometry: new FrustumGeometry({
                frustum: newObj as PerspectiveFrustum,
                origin: this.viewPosition,
                orientation: orientation,
            }),
        });
        const p1s = new Primitive({
        ...
        this.videos.push(p1s);
        this.videos.push(p2s);
    }
    ...
    public clear() {
        ...
        if (this.frustumOutlineGeometry && !this.frustumOutlineGeometry.isDestroyed()) {
    ...
    private getCurrentMousePosition(
        scene: Scene,
        position: Cartesian2,
        noPickEntity?: ReturnType<typeof scene.pick>,
    ) {
        ...
        if (scene.pickPositionSupported && defined(pickedObject)) {
            ...
            if (noPickEntity == null || (noPickEntity && entity !== noPickEntity)) {
                ...
                if (defined(cartesian)) {
                    ...
                    if (!defined(pickedObject.id) && height >= -500) return cartesian;
                }
            }
        }
